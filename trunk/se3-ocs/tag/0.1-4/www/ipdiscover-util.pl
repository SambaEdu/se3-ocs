...
# my $dbuser = 'ocs';
# my $dbpwd = 'ocs';
my $dbuser=`cat /var/www/se3/includes/dbconfig.inc.php | grep COMPTE_BASE | cut -d= -f2 | cut -d\\" -f2`;
my $dbpwd=`cat /var/www/se3/includes/dbconfig.inc.php | grep PSWD_BASE | cut -d= -f2 | cut -d\\" -f2`;
my $dbhost=`cat /var/www/se3/includes/dbconfig.inc.php | grep SERVEUR_SQL | cut -d= -f2 | cut -d\\" -f2`;
my $db = 'ocsweb';
my $dbp = '3306';
#
chomp($dbuser);
...
my $request;
my $row;
my $dbh = DBI->connect("DBI:mysql:database=$db;host=$dbhost;port=$dbp", $dbuser, $dbpwd)
 or die $!;
